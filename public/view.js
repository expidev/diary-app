...
export async function displayList() {
    try {
        const diaryEntries = await DiaryController.getDiaries();
...
function showDiaryItem(entries, container, i) {
    try {
        if (i >= entries.length)
            return;
        ...
        const contentElement = document.getElementById(`diary-content-${entry.id}`);
        const toggleButton = document.getElementById(`toggle-content-${entry.id}`);
        ...
        const updateButton = entryElement.querySelector(`#update-entry-${entry.id}`);
        updateButton.addEventListener('click', async () => await handleUpdate(entry.id));
        const deleteButton = entryElement.querySelector(`#delete-entry-${entry.id}`);
